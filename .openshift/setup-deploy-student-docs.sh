...
oc -n "${NS_WORKBENCH}" process -f "${LABS_STUDENT_DEPLOYMENT_TEMPLATE_PATH}" -l "app=ocp101-labs-${STUDENT_ID}" -l "student=${STUDENT_ID}" -p "HOST=ocp101-labs-${NS_WORKBENCH}${CLUSTER_APPS_URL}" -p "NAME=ocp101-labs" -p "SUFFIX=-${STUDENT_ID}" -p "IMAGE_NAMESPACE=${NS_WORKBENCH}" -p "IMAGE_NAME=ocp101-labs:latest" -p "STUDENT_ID=${STUDENT_ID}" -p "NAMESPACE_TOOLS=${NS_TOOLS}" -p "NAMESPACE_DEV=${NS_DEPLOY}" | oc -n "${NS_WORKBENCH}" apply -f - --overwrite=true
